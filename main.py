...
from threading import Thread # Render uchun qo'shildi
import os # Portni aniqlash uchun qo'shildi

# --- FLASK SERVER (RENDER UCHUN UXLAMASLIK QISMI) ---
app = Flask('')

@app.route('/')
def home():
    return "Bot status: ONLINE (O'zbekiston vaqti bilan)"

def run():
    # Render avtomatik beradigan PORT ni ishlatamiz, bo'lmasa 8080
...
BOT_TOKEN = "8749296120:AAEeLAmghz_BLK7lr6kAlzqdxk7iDU2zEhA" 

ALLOWED_PHONES = ["+998882855507", "+998770014411"] 
NAMES = ["Umid", "Asqar"]
...
@dp.message(Anketa.samokat_num, F.text)
...
async def main():
    # Render serverini bot bilan parallel ravishda ishga tushirish
    keep_alive()
